...
module Quickstrom.CLI.Reporter.JSON where
...
newtype Test = Test {transitions :: Transitions}
  deriving (Eq, Show, Generic, JSON.ToJSON)
...
newtype JSONReporterException = JSONReporterException Text
  deriving (Show, Eq)

instance Exception JSONReporterException

jsonReporter :: (MonadReader Quickstrom.LogLevel m, MonadIO m) => FilePath -> Quickstrom.Reporter m
jsonReporter reportDir = Quickstrom.Reporter {preCheck, report}
  where
    ...
    report _webDriverOpts checkOpts result = do
      ...
      whenM (liftIO (Directory.doesPathExist reportDir)) $
        throwIO (JSONReporterException "File or directory already exists, refusing to overwrite!")
...
newtype ScreenshotFileException = ScreenshotFileException Text
  deriving (Show)
