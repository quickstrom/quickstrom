...
foreignFunction :: (MonadError (EvalError EvalAnn) m, ForeignFunctionArity f, EvalForeignFunction m f) => f -> ForeignFunction m
foreignFunction (f :: f) =
  ForeignFunction
...
foreignFunctionArityMismatch :: MonadError (EvalError EvalAnn) m => SourceSpan -> m a
...
class Monad m => EvalForeignFunction m f where
  evalForeignFunction :: MonadError (EvalError EvalAnn) m => f -> SourceSpan -> [Value EvalAnn] -> m (Value EvalAnn)
...
class ToHaskellValue r where
  toHaskellValue :: MonadError (EvalError EvalAnn) m => SourceSpan -> Value EvalAnn -> m r
...
foreignFunctions :: MonadError (EvalError EvalAnn) m => Map (Qualified Ident) (ForeignFunction m)
foreignFunctions =
  Map.fromList
    [ (qualifiedName "Control.Bind" "arrayBind", foreignFunction arrayBind :: ForeignFunction m)
      -- ,
